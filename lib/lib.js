...
function initBoard () {
  if (!tests.boardValid() || !tests.cellsValid()) {
    // displayMessage(msg,id) takes a message parameter and an html id tag, and returns it to the id tag in the browser. Seems to be built-in js function?
  ...
  //   eg var fruit = ['cherries', 'apples', 'bananas'];
  ...
  // .sort(compareFunction) ie can specify a function that defines the sort order. Returns the sorted array 'in place', ie no copy is made.
  board.cells.sort(cellCompare)
  var boardNode = document.getElementsByClassName('board')[0]
  // var board is an object containing one array called 'cells'. Within the "cells" array we have 9 objects. However board isn't an array so why use [0]?
  drawBoard(boardNode)
  addListeners(boardNode)
  return true
}

// Draw board based on number of cells and an assumption about how much space should be allowed for each cell.
// uses boardNode defined in displayMessage above.
